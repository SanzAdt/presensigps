...
class KonfigurasiController extends Controller
{
    ...
    public function setjamkerja($nis)
    {
        ...
        $jamkerja = DB::table('jam_kerja')
            ...
            ->get();
        $cekjamkerja = DB::table('konfigurasi_jamkerja')
            ->where('nis', $nis)
            ->count();
        if ($cekjamkerja > 0) {
            $setjamkerja = DB::table('konfigurasi_jamkerja')
                ->where('nis', $nis)
                ->get();
    ...
    public function updatesetjamkerja(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            DB::table('konfigurasi_jamkerja')->where('nis', $nis)->delete();
            Setjamkerja::insert($data);
            DB::commit();
            return redirect('/karyawan')->with(['success' => 'Jam Kerja Berhasil Di Setting']);
        } catch (\Exception $e) {
